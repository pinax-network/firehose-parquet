...
`state_changes` is unchanged. Firehose NEAR blocks carry state changes only at block level, where they are empty on live blocks, and the shard data has none.
