...
- Add/change CLI flag or subcommand:
  - `firehose-parquet/src/cli.rs` (shared flags/subcommands including `BuildArgs` for `fireparq build`); `src/cli/configuration.rs` for config conversion and value validation
