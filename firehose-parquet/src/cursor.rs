...
pub fn save_cursor_parquet(path: &Path, state: &CursorState) -> anyhow::Result<()> {
    save_cursor_parquet_with_sync(path, state, sync_dir)
}

fn save_cursor_parquet_with_sync(
    path: &Path,
    state: &CursorState,
    ...
    sync_directory(dir)
        .with_context(|| format!("syncing cursor directory {} after rename", dir.display()))?;
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn directory_sync_failure_is_a_save_error() {
        let dir = TempDir::new().unwrap();
        let error = save_cursor_parquet_with_sync(
            &dir.path().join(CURSOR_PARQUET_FILENAME),
            &CursorState::default(),
