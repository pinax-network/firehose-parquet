...
use mapper::CosmosBlockMapper;
use std::path::PathBuf;
use std::time::Instant;
use tracing::info;

#[derive(Parser, Debug)]
#[command(name = "firehose-cosmos-to-parquet", version, about = "Convert Firehose Cosmos gRPC stream to Apache Parquet")]
struct Cli {
...
async fn main() -> Result<()> {
    ...
    info!(?config, "starting Cosmos pipeline");
    ...
    let mut mapper = CosmosBlockMapper::new(include_fork_step, encode_bytes);
    ...
    info!(blocks_processed, "Cosmos pipeline finished");
    Ok(())
}
