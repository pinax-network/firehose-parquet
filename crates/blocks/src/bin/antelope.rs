...
#[derive(Parser, Debug)]
#[command(name = "firehose-antelope-to-parquet", version, about = "Convert Firehose Antelope gRPC stream to Apache Parquet")]
struct Cli {
    #[command(subcommand)]
...
async fn main() -> Result<()> {
    ...
    info!(?config, "starting Antelope pipeline");
    ...
    client
        .stream_blocks(|block_bytes, _cursor, identity: BlockIdentity, step: i32| {
            ...
            let block_num = identity.block_num;
            let ts = identity.timestamp;
            min_block = Some(min_block.map_or(block_num, |s: u64| s.min(block_num)));
            max_block = Some(max_block.map_or(block_num, |s: u64| s.max(block_num)));
            if let Some(t) = ts {
            ...
            if blocks_processed % 100 == 0 {
                info!(blocks_processed, block_num, buffered_rows = mapper.max_table_rows(), "progress");
            }
