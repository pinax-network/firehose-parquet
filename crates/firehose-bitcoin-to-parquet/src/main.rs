...
#[derive(Parser, Debug)]
#[command(name = "firehose-bitcoin-to-parquet", version, about = "Convert Firehose Bitcoin gRPC stream to Apache Parquet")]
struct Cli {
...
async fn main() -> Result<()> {
    ...
    client
        .stream_blocks(|block_bytes, _cursor| {
            let block_height = prost::Message::decode(block_bytes.as_slice())
                .map(|b: proto::btc::Block| b.height as u64)
                .unwrap_or(0);
    ...
    info!(blocks_processed, "Bitcoin pipeline finished");
    Ok(())
}
