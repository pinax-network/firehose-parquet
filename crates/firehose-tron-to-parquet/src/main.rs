...
use mapper::TronBlockMapper;
use std::path::PathBuf;
use std::time::Instant;
use tracing::info;

#[derive(Parser, Debug)]
#[command(name = "firehose-tron-to-parquet", version, about = "Convert Firehose Tron gRPC stream to Apache Parquet")]
struct Cli {
    ...
    #[arg(long, default_value = "tron_base58")]
    encode_bytes: String,
}
...
async fn main() -> Result<()> {
    ...
    info!(?config, "starting Tron pipeline");

    let final_blocks_only = config.final_blocks_only;
    let include_fork_step = !final_blocks_only;
    let encode_bytes = parse_encode_bytes(&cli.encode_bytes).unwrap_or(EncodeBytes::TronBase58);
    let mut mapper = TronBlockMapper::new(include_fork_step, encode_bytes);
    ...
    info!(blocks_processed, "Tron pipeline finished");
    Ok(())
}
